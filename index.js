...
const vcard = 'BEGIN:VCARD\n' // metadata of the contact card
            ...
            + 'TEL;type=CELL;type=VOICE;waid=553184903469:+55 31 8490-3469\n' // WhatsApp ID + phone number
            + 'END:VCARD'
/******END OF VCARD INPUT******/
...
async function starts() {
	...
	client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `Olá 🤞 @${num.split('@')[0]}\nBem vindo ao grupo, aqui é só humor and piadas  *${mdata.subject}*`
				...
				teks = `Flw @${num.split('@')[0]} Não aguentou a pressão dos cria👋`
				let buff = await getBuffer(ppimg)
				client.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
	...
	client.on('chat-update', async (mek) => {
		try {
			...
			mess = {
				wait: '⌛ Espere, o comando está sendo executado, caso não envie, tente novamente⌛',
				success: '✔️ Sucesso ✔️',
				...
				only: {
					group: '[❗] Comando só pode ser usado nos grupos! ❌',
					ownerG: '[❗] Este comando só pode ser usado pelo dono do grupo! ❌',
					ownerB: '[❗] Este comando só pode ser usado pelo henriq né novinha ❌',
			...
			const ownerNumber = ["553184903469@s.whatsapp.net"] // replace this with your number
			...
			const isOwner = ownerNumber.includes(sender)
                        ...
                        const NomerOwner = '553184903469@s.whatsapp.net'
                        /******ApiKey Input******/
               ...
               break
				case 'demote':
                  ...
                  case 'promote':
					...
					if (!isGroupAdmins) return reply(mess.only.admin)
					if (!isBotGroupAdmins) return reply(mess.only.Badmin)
					if (mek.message.extendedTextMessage === undefined || mek.message.extendedTextMessage === null) return reply('Marque alguém para receber a promoção')
					mentioned = mek.message.extendedTextMessage.contextInfo.mentionedJid
					if (mentioned.length > 1) {
						...
						mentions(`Comando recebido, adicionar posição como administrador : @${mentioned[0].split('@')[0]}`, mentioned, true)
						client.groupMakeAdmin(from, mentioned)
					}
					break
				  case 'wa.me':
  ...
  client.updatePresence(from, Presence.composing) 
    ...
    client.sendMessage(from, options, text, { quoted: mek } )
			...
			case 'quotes':
              ...
              case 'owner':
                case 'creator':
                  client.sendMessage(from, {displayname: "Henriq rei delas tbm conhecido como refrizin", vcaurd: vcaurd}, MessageType.contact, { quoted: mek})
               client.sendMessage(from, 'Aqui está o número do dono 553184903469',MessageType.text, { quoted: mek} )
                break
	case 'hidetag':
                client.updatePresence(from, Presence.composing) 
                if (!isUser) return reply(mess.only.daftarB)
		...
		case 'spamcall':
               client.updatePresence(from, Presence.composing)
				...
				case 'wikien':
					...
					break				
				case 'ytmp3':
					if (args.length < 1) return reply('Tocar o quê?')
				...
				case 'ytmp4':
					...
					if (anu.error) return reply(anu.error)
					teks = `*❏ Titulo* : ${anu.title}\n\n*O VIDEO ESTÁ SENDO ENVIADO, NAO FAÇA SPAM*`
					thumb = await getBuffer(anu.thumb)
					client.sendMessage(from, thumb, image, {quoted: mek, caption: teks})
					buffer = await getBuffer(anu.result)
			...
			tels = body.slice(6)
               ...
               if (data.error) return reply(data.error)
				...
				case 'block':
					...
					client.sendMessage(from, `Contato bloqueado ${body.slice(8)}@c.us`, text)
					break
				case 'hilih':
					client.updatePresence(from, Presence.composing) 
          ...
          contextInfo: { mentionedJid: [from] }
           ...
           break
				...
				case 'add':
					...
					if (!isGroupAdmins) return reply(mess.only.admin)
					if (!isBotGroupAdmins) return reply(mess.only.Badmin)
					if (args.length < 1) return reply('A que você deseja adicionar um genio??')
				...
				case 'ban':
					...
					if (!isGroupAdmins) return reply(mess.only.admin)
					if (!isBotGroupAdmins) return reply(mess.only.Badmin)
					if (mek.message.extendedTextMessage === undefined || mek.message.extendedTextMessage === null) return reply('Banir quem?')
					mentioned = mek.message.extendedTextMessage.contextInfo.mentionedJid
					if (mentioned.length > 1) {
						...
						mentions(`Comando recebido, vaza daqui diabo  : @${mentioned[0].split('@')[0]}`, mentioned, true)
						client.groupRemove(from, mentioned)
					client.sendMessage(mentioned, 'bruh', text)
					}
					break
				case 'exe':
	              client.updatePresence(from, Presence.composing) 
				...
				case 'ocr':
					...
					break
                     ...
                     const bug = body.slice(5)
                      if (pesan.length > 300) return client.sendMessage(from, 'Desculpe, o texto é muito longo, maximo de 300 textos', msgType.text, {quoted: mek})
                    ...
                    reply('O problema foi relatado ao proprietário do BOT, relatorio falso/main2 nao sera respondido.')
                    break
               case 'apakah':
               client.updatePresence(from, Presence.composing) 
	    ...
	    case 'kapankah':
			...
			case 'fechar':
                ...
                case 'abrir':
					...
					if (!isBotGroupAdmins) return reply(mess.only.Badmin)
					open = {
					text: `Grupo aberto pelo administrador @${sender.split("@")[0]}\nagora *os membros comuns* também podem falar `,
					contextInfo: { mentionedJid: [sender] }
					}
				...
				case 'toimg':
                	...
                	client.updatePresence(from, Presence.composing) 
                        if (!isUser) return reply(mess.only.daftarB)
					if (!isQuotedVideo) return reply('❌ responda o vídeo um ❌')
					reply(mess.wait)
					encmedia = JSON.parse(JSON.stringify(mek).replace('quotedM','m')).message.extendedTextMessage.contextInfo
					media = await client.downloadAndSaveMediaMessage(encmedia)
					ran = getRandom('.mp4')
					exec(`ffmpeg -i ${media} ${ran}`, (err) => {
						...
						buffer = fs.readFileSync(ran)
						client.sendMessage(from, buffer, audio, {mimetype: 'áudio/mp4', quoted: mek})
						fs.unlinkSync(ran)
					})
					break
               ...
               if (anu.error) return reply(anu.error)
                ...
                break
                   ...
                   break
					...
					if (args.length < 1) return reply(`Parâmetro Errado\nComando : ${prefix}registrar nome | idade\nExemplos : ${prefix}registrar João|17`)
					var reg = body.slice(10)
            ...
            break
                                ...
                                case 'nsfwtrap':
                                        try{
                                                if (!isNsfw) return reply('❌ *Chat SFW* ❌')
                                                if (!isUser) return reply(mess.only.daftarB)
                                                res = await fetchJson(`https://tobz-api.herokuapp.com/api/nsfwtrap?apikey=BotWeA`, {method: 'get'})
                                                buffer = await getBuffer(res.result)
                                                client.sendMessage(from, buffer, image, {quoted: mek, caption: 'Esse curte traveco slc'})
                                        } catch (e) {
                                                console.log(`*Error* :`, color(e,'red'))
                                                reply('❌ *ERROR* ❌')
                                        }
				...
				case 'clone':
					...
					break
                                ...
                                case 'pubglogo':
                                        ...
                                        var teks1 = gh.split("|")[0];
                                        var teks2 = gh.split("|")[1];
                                        if (args.length < 1) return reply('onde está o texto um \ nExemplos: ${prefix}pubglogo Nazwa|Canss')
                                        if (!isUser) return reply(mess.only.daftarB)
                                        ...
                                        buffer = await getBuffer(anu.result)
                                        client.sendMessage(from, buffer, image, {quoted: mek, caption: 'Aqui está o logotipo, mana...'})
                                        break
                                case 'herrypotter':
                                case 'harrypotter':
                                        var gh = body.slice(12)
